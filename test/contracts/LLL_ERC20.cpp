...
(seq
  ...
  ;; --------------------------------------------------------------------------
  ;; Will revert if sent any Atp. We use the macro immediately so as
  ;;   to abort if sent any Atp during contract deployment.
...
BOOST_AUTO_TEST_CASE(transfer_from)
{
	...
	// Send account(1) some atp for gas.
	sendEther(account(1), 1000 * atp);
	BOOST_REQUIRE(balanceAt(account(1)) >= 1000 * atp);
